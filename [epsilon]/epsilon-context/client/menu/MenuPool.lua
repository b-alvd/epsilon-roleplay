...
local function NewMenuPool()
	...
	self.keys = {
		keyboard = {
			holdForCursor = 19,   -- Alt gauche
			click         = 24,   -- Clic gauche : ouvre le menu OU active un item
		}
	}
	...
	local function Process()
		...
		if (IsControlPressed(0, self.keys.keyboard.holdForCursor) or IsDisabledControlPressed(0, self.keys.keyboard.holdForCursor)) then
			...
			if (IsDisabledControlJustPressed(0, self.keys.keyboard.click)) then
				...
				elseif (self.OnInteract) then
					-- Menu fermé : clic gauche = ouvrir le menu via raycast
					Citizen.CreateThread(function()
						self.OnInteract(screenPosition, hitSomething, worldPosition, hitEntityHandle, normalDirection)
					end)
				end
			elseif (IsDisabledControlJustReleased(0, self.keys.keyboard.click)) then
				for i = #self.menus, 1, -1 do
					if (self.menus[i]:Visible() and self.menus[i]:InBounds(cursorPosition)) then
						self.menus[i]:Released(cursorPosition)
						break
					end
				end
			end
